...
class SelectionTable(QtWidgets.QTableWidget):
	...
	#	self.verticalHeader().hide()
		item=QtWidgets.QTableWidgetItem("Modes")
		item.setBackground(QtGui.QColor(255, 255, 255))
		self.setHorizontalHeaderItem(0,item)
		self.setFrameStyle(QtWidgets.QFrame.NoFrame)
		self.horizontalHeader().setDefaultSectionSize(0)
		...
		item.setTextAlignment(QtCore.Qt.AlignCenter)
	#	self.setStyleSheet("""QWidget {border-top: 1px solid black;border-left: 1px solid black;background-color: rgb(255, 255, 255);}""")
...
#	try:
	for i,each in enumerate(seqlist):
		...
		cmd=disttbpath+' -q 0 -E 2 -V -1.53 -s 0.0 -W 6 -O -C 4-4 -b 62 -g 0 -f -1.53 -Q 100.0 -h 0 -F -X 0.1 -x 1000 -i ' +os.path.join(outpath,seqdict[each][0].split(";")[0])
...
#	print parstring
	...
	#	print name
		try:
			if subsetval!=0:
				if v==0:
					sampleids[name.split("_all.fa")[0].split(".")[0]]=subset_bylength(os.path.join(outpath,indir,name),os.path.join(outpath,indir2,name),subsetval,plen,postdemlen)
					cmd=disttbpath+' '+parstring+' -i '+os.path.join(outpath,indir2,name) 
				if v==1:
					sampleids[name.split("_all.fa")[0].split(".")[0]]=subset_bylength(os.path.join(indir,name),os.path.join(outpath,indir2,name),subsetval,plen,postdemlen)
					cmd=disttbpath+' '+parstring+' -i '+os.path.join(outpath,indir2,name) 
			else:
				cmd=disttbpath+' '+parstring+' -i '+os.path.join(outpath,indir2,name)  
			with open(os.devnull, 'w') as devnull:
		...
		result=[transcheck,conseqs,flags,coverages]
		return result
def pool_init(queue):
	runconsensusparts.queue = queue
...
class MSAcheck(QtCore.QThread):
	...
	def run(self):
		...
		ambiguity_codes=[("R", "A"), ("R", "G"),("M", "A"), ("M", "C"),("S", "C"), ("S", "G"),("Y", "C"), ("Y", "T"),("K", "G"), ("K", "T"),("W", "A"), ("W", "T"),("V", "A"), ("V", "C"),("V", "G"),("H", "A"),("H", "C"),("H", "T"),("D", "A"), ("D", "G"),("D", "T"), ("B", "C"),("B", "G"), ("B", "T"),("N", "A"), ("N", "G"), ("N", "C"), ("N", "T")]
		if self.ngood>=3:
			cmd=disttbpath+' -q 0 -E 2 -V -1.53 -s 0.0 -W 6 -O -C 4-4 -b 62 -g 0 -f -1.53 -Q 100.0 -h 0 -F -X 0.1 -x 1000 -i ' +os.path.join(self.outpath,"barcodesets","temps",self.goodfile) 
		...
		ngoodbarcodes=0
	#	print "good corlist",self.ngood,self.tocorlist
...
class OptWindow(QtWidgets.QWidget):
	...
	def setdemoutpath(self):
		try:
			...
			pass
	#	self.mygetdemoutpath=messagebox_demoutpath()
	#	self.mygetdemoutpath.selectdirectory.clicked.connect(self.setdemoutpath2)
	#	self.mygetdemoutpath.defaultdirectory.clicked.connect(self.setdemoutpath3)
...
#		self.logfile=open(os.path.join(self.demoutpath,"log.txt"),'w')
	...
	#	print "skipping consensus"
		if self.flag==3:
			...
			self.statuswidget.pbar.setMaximum(len(os.listdir(self.indir)))
			self.statuswidget.pbar.setFormat("%v"+"/"+str(len(os.listdir(self.indir))))
			self.mycounterdemreads.notifyProress.connect(self.updatecoveragecalcbar)
			self.mycounterdemreads.taskFinished.connect(self.updateconsensussetup)
			self.startcalcdemreads()
	def updatecoveragecalcbar(self,i):
		self.statuswidget.pbar.setValue(i)
	def updateconsensussetup(self,i):
		self.updateconsensus()
	def terminate(self):
	#	print "changetabtab"
	...
	def setforconsensusonly(self):
		...
		self.statuswidget.pbarTitle.setText("Calculating raw coverages")
		self.minlen=int(self.MinLengthTextBox.text())
		self.explen=int(self.ExpLengthTextBox.text())
		...
		self.statuswidget.pbar.setFormat("%v"+"/"+str(len(os.listdir(self.indir))))
		self.mycounterdemreads=calculatecoverage(self.indir)
		self.mycounterdemreads.notifyProgress.connect(self.updatecoveragecalcbar)
		self.mycounterdemreads.taskFinished.connect(self.completecalccoverage)
		self.startcalcdemreads()
	def startcalcdemreads(self):
	#	print "startcalc"
		self.mycounterdemreads.start()
	def completecalccoverage(self):
		print "isdone!"
	...
	def printreadfasta(self,v):
		...
		if self.consensuslenstat==True:
			...
			self.statuswidget.pbar.setValue(0)
			...
			def chunker_list(seq, size):
				return [seq[i::size] for i in range(size)]
			nparts=chunker_list(self.inlistforconsensus,len(self.inlistforconsensus))
			...
			for i,j in enumerate(nparts):
				self.pool.apply_async(func=runconsensusparts, args=(j,),  callback=addresult)
			self.pool.close()
			self.timer = QtCore.QTimer()
			self.timer.timeout.connect(self.updateconsensus)
			self.timer.start(20)
		else:
			self.updateconsensus()
	def runcon1(self):
	...
	def updatecon1(self,i):
		self.statuswidget.pbar.setValue(len(self.resultlist))
	def wait(self):
		self.demoutpath
	...
	#	print result
		...
		if self.consensuslenstat==True:
		#	self.resultlist.append(result)
			if len(self.resultlist)!=self.counter:
	...
	def callsecondconsensus(self,i):
		...
		#	os.mkdir(os.path.join(self.demoutpath,"2b_ConsensusBySimilarity","90perc_200"))
			...
			def chunker_list(seq, size):
				return [seq[i::size] for i in range(size)]
		#	self.nsampledem=len(partlist)
			nparts=chunker_list(partlist,len(partlist))
			def addresult(result):
				self.resultlist.append(result)
			...
			self.pool.close()
			self.timer = QtCore.QTimer()
			self.timer.timeout.connect(self.updateconsensus2)
			self.timer.start(20)
		else: 
	...
	def fixbarcodessetup(self):
		...
		def addresult(result):
			self.resultlist.append(result)
		self.queue = multiprocessing.Queue()
		self.pool = multiprocessing.Pool(processes=4, initializer=pool_init2, initargs=(self.queue,))
		for i,j in enumerate(nparts):
			self.pool.apply_async(func=runtoptwenty, args=(j,),  callback=addresult)
		self.pool.close()
		self.timer = QtCore.QTimer()
		self.timer.timeout.connect(self.updateProgressfix)
		self.timer.start(1)
	def updateProgressfix(self):
	#	self.resultlist.append(result)
		if len(self.resultlist)!=self.nsamplefix:
			#print self.resultlist
			#if self.queue.empty(): return
			self.statuswidget.pbar.setValue(len(self.resultlist))
		#	self.updateProgressfix()
		else:
			self.timer.stop()
	...
	#	self.statuswidget.logstatus.textCursor().insertHtml("The pipeline is completed. "+str(self.nfinal)+ " consensus barcodes have been built. <br>Please check "+os.path.join(self.demoutpath,"Main_barcode_results")+ " for the final output")
	...
	def messagebox2(self,string):
		...
		self.mb.addButton(defaultdirectory, QtWidgets.QMessageBox.NoRole)
		self.mb.setWindowTitle("ONTBarcoder")
		self.selectionresult=self.mb.exec_()
